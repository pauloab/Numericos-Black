 ...
 */
public class VistaPuntoMedioController implements Initializable {
        ...
        btProcesar.setOnMouseClicked(event -> {
            ...
            if (Matematico.validarExpresionDosVariables(funcion)) {
                ...
                if (x0 != null && x1 != null && b != null && h != null && !error) {
                    euler = new Euler(x0, x1, b, funcion, h);
                    try {
                        euler.euler();
                        String[] headers = {"x", "f(x,y)", "y"};
                        Graficos.cargarEnTableView(tvResultados, euler.getDatos(), headers);
                    } catch (Exception ex) {
                        error = true;
    ...
    private void Graficar() {
        try {
            String[][] datos = euler.getDatos();
            CoordinatePair[] puntos = new CoordinatePair[datos.length];
            for (int i = 0; i < datos.length;i++) {
                puntos[i] = new CoordinatePair(Double.parseDouble(datos[i][0]), Double.parseDouble(datos[i][2]));
            }
